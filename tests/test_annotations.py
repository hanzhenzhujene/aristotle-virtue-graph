...
def test_processed_export_succeeds_for_repository_approved_seed(tmp_path: Path) -> None:
    ...
    assert len(graph["passages"]) == 45
    assert stats["concept_count"] == 26
    assert stats["relation_count"] == 21
    assert stats["passage_count"] == 45
    assert stats["concept_review_statuses"] == {"approved": 26}
    assert stats["relation_review_statuses"] == {"approved": 21}
    assert len(approved_passages) == 45
    assert graphml.number_of_nodes() == 26
    assert graphml.number_of_edges() == 21
